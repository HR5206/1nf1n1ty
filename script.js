...
async function signup(){
  ...
  const hp = await hash(password);
  const user = { email, pass: hp, created_at: new Date().toISOString(), bio: 'Hey there! I\'m using SastraDaily.', avatar_url: await makeAvatarFromEmail(email) };
...
async function renderProfile(){
  ...
  document.getElementById('profileEmail').textContent = me.email;
  document.getElementById('profileBio').textContent = u.bio || "Hey there! I'm using SastraDaily.";
  if(u.avatar_url){ document.getElementById('profileAvatar').src = u.avatar_url; }
